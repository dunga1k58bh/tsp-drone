...
for k in K:
    for j in N:
        mdl.add_constraint(T[k, 0] >= w[j] * mdl.sum(y[c, 0, j] for c in C))
...
#24
for k in K:
    mdl.add_constraint(T[k, n+1] >= T[k, 0] + mdl.sum(t[i,j]*x[k, i, j] for i in N_0n1 for j in N_0n1) + sum(e[k, i] for i in D))
